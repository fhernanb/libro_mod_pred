...
Los árboles de regresión fueron propuestos par [Leo Breiman](https://machinelearningmastery.com/classification-and-regression-trees-for-machine-learning/) y son árboles de decisión que tienen como objetivo asignar un valor de $\hat{y}$ dependiendo de los valores de las covariables.

Un árbol de regresión consiste en hacer preguntas como ¿$x_k \leq c$? para cada una de las covariables, de esta forma el espacio de las covariables es divido en hiper-rectángulos y todas las observaciones que queden dentro de un hiper-rectángulo tendrán el mismo valor $\hat{y}$.
...
```{r ilustra_arbol_reg01, echo=F, fig.cap='Ilustración de la técnica Árboles de Regresión. A la izquierda el árbol y a la derecha la partición del espacio.', dpi=60, fig.align='center'}
...
Robust.
- Performs well even if you deviate from assumptions Scales to big data

```{block2, type='rmdtip'}
Para conocer más sobre el tema se recomienda ver [este video](https://www.youtube.com/watch?v=7VeUPuFGJHk) y también leer este [documento](https://rafalab.github.io/pages/649/).
```
...
```{r tree01, fig.align='center', fig.height=4, fig.width=4}
plot(mod2)
text(mod2, pretty=0)
```
