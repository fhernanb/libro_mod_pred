...
En este ejemplo vamos a usar los mismos datos de las casas en Boston para predecir la variable medv en función de diferentes características socioeconómicas, ambientales y estructurales de las viviendas. La base de datos `Boston` del paquete `MASS` de R contiene las variables crim (tasa de criminalidad per cápita por localidad), zn (proporción de terrenos residenciales destinados a lotes mayores de 25,000 pies cuadrados), indus (proporción de acres de negocios no minoristas por localidad), chas (variable indicadora del río Charles, donde 1 indica que la vivienda limita con el río y 0 en caso contrario), nox (concentración de óxidos nítricos en partes por 10 millones), rm (número promedio de habitaciones por vivienda), age (proporción de viviendas ocupadas por sus propietarios construidas antes de 1940), dis (distancia ponderada a cinco centros de empleo en Boston), rad (índice de accesibilidad a carreteras radiales), tax (tasa de impuesto a la propiedad por cada $10,000 del valor), ptratio (relación alumno-profesor en cada localidad), black (índice calculado como 1000(Bk − 0.63)^2, donde Bk representa la proporción de población afrodescendiente), lstat (porcentaje de la población de bajo estatus socioeconómico) y medv (valor mediano de las viviendas ocupadas por sus propietarios en miles de dólares) usando la base de datos Boston del paquete **MASS** de @R-MASS.
...
A continuación vamos a obtener medidas de desempeño para el modelo entrenado usand la base de datos de validación y el paquete **yarrdstick** de @R-yardstick.

```{r}
library(yardstick)

predictions_rf <- predict(model, newdata = test_data, type="response")

# Para calcular el MSE
mse_rf <- mse_vec(truth=test_data$medv, estimate=predictions_rf)
mse_rf

# Para calcular el MAE
mae_rf <- mae_vec(truth=test_data$medv, estimate=predictions_rf)
mae_rf
```

Para tener una idea de la grandeza de los valores de MSE y MAE anteriores, vamos a ajustar dos modelos alternativos, un modelo regresión lineal simple y un árbol de regresión (sencillo).
...
library(rpart)
model_tree <- rpart(medv ~ ., data = train_data)
...
mse_lm   <- mse_vec(truth=test_data$medv, estimate=predictions_lm)
mse_tree <- mse_vec(truth=test_data$medv, estimate=predictions_tree)

mae_lm   <- mae_vec(truth=test_data$medv, estimate=predictions_lm)
mae_tree <- mae_vec(truth=test_data$medv, estimate=predictions_tree)

# Mostrar resultados
results <- data.frame(
  Modelo = c("Regresión Lineal", "Árbol de Regresión", "Random Forests"),
...
De la salida anterior podemos ver que el modelo de random forest tiene mejores medidas de desempeño que los otros dos modelos de machine learning.
...
**distree** es un paquete de R propuesto por @R-disttree para ajustar árboles y bosques de regresión distribucionales basado en la estimación de máxima verosimilitud de parámetros para familias de distribución específicas. Para instalar el paquete puedes usar el siguiente código.
...
El argumento `family` se utiliza para indicar la distribución estadística que se asume para la variable respuesta $Y$. Se puede elegir cualquier distribución de **gamlss** de @R-gamlss o cualquier distribución de los paquetes **RelDists** de @R-RelDists, **RealDists** de @R-RealDists y **DiscreteDists** de @R-DiscreteDists.
