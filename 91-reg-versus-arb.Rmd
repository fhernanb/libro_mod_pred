...
with(datos_train, plot(x=x, y=y, pch=20))
```
...
Ahhhhhh, ¿que está sucediendo? Ahora el modelo de regresión lineal simple se comporta mejor, ¿cómo se explica eso?

Ahora vamos a realizar el estudio de simulación, vamos a explorar el efecto de $n = 10, 20, 40$ sobre el $ECM$ usando 5 réplicas para cada $n$, un estudio de simulación "naive" pero ilustrativo.
...
```{r message=FALSE}
library(dplyr)
result %>% group_by(n) %>% summarise(ecm_lrs=mean(ecm_lrs),
                                     ecm_arb=mean(ecm_arb))
```

## Retos
A continuación los retos que usted debe aceptar.

1. Extienda el estudio de simulación para otros valores de $n$.
2. Con el estudio de simulación extendido haga un gráfico de $ECM$ promedio versus $n$ para rls y árboles en la misma figura.
3. ¿Cuál $ECM$ es menor?
3. Mirando la figura anterior, ¿se consigue un $ECM$ de cero para un $n$ suficientemente grande?
